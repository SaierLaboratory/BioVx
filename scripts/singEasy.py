...
if __name__ == '__main__':
    ...
    parser.add_argument( '-d', '--tabledata', type = str, dest = 'data', required = True, metavar = '<GBlast_result.tsv>', help = 'MANDATORY. the file name or file name with the absolute address of the GBlast table results of a genome in tsv format.' )
    parser.add_argument( '-db', '--database', type = str, dest = 'db', required = True, metavar = '<tcdb.faa>', help = 'MANDATORY. File containing all tcdb sequences in fasta format.')
    parser.add_argument( '-ad', '--address', type = str, dest = 'ad', required = True, metavar = '<path/to/xgbhit>', help = 'The address of the xgbhit directory containing all plots generated by GBlast.' )
    parser.add_argument( '-e', '--evalue', type = float, dest = 'evalue', default = '1e-10', help = 'The evalue filter of trusting a GBlast hit as a good hit. The default boundary is 1e-10.')
    parser.add_argument( '-hcov', '--subjectcoverage', type = float, dest = 'hcov', default = 70.0, help = 'The alignment coverage filter for subjects. Default is 70 percent.')
    parser.add_argument( '-qcov', '--querycoverage', type = float, dest = 'qcov', default = 70.0, help = 'The alignment coverage filter for queries. Default is 70 percent.')
    parser.add_argument( '-ds', '--domainshared', type = int, dest = 'dshare', default = 1, help = 'The minimum number of common domains in both queries and subjects. Default is 1.')
