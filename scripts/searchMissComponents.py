...
def get_output( display ,dcov, overwrite, query, dbname, evalue, coverage, which, e_filter, out, alignment_length ):
	...
	for index, row in df.iterrows():
		...
		outfile.write( '<center><table style = "width:100%" border = "0"><tr><td><center><img src = "{p}/{q}.png" style="width:90%; height:90%" /></center></td><td><center><img src = "{p}/{s}.png" style="width:90%; height:90%" /></center></td></tr><tr><td colspan = "2"><center><img src = "{p}/{qs}.png" style="width:50%; height:50%" /></center></td></tr></table></center>'.format( p=plot_path, q=row['qacc']+'_vs_'+row['sacc'], s=row['sacc']+'_vs_'+row['qacc'],qs=row['qacc']+'_'+row['sacc']+'_aligned' ) )
		# insert the hmm info
...
def main():
	...
	parser.add_argument( '-k', '--keyword', type = str, dest = 'keywords', metavar = '<keywords string>', help = 'MANDATORY. Descriptions of the missing components for searching on the NCBI website. Logical expressions like "AND","OR" can be used for detailed description.' )
	...
	parser.add_argument( '-g', '--genome', type = str, dest = 'genome', metavar = '<string input_file>', help = 'MANDATORY. Name or location of a complete genome file in FASTA format or FASTA compressed format(bz,gz).' )
